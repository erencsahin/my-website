...
/* function displayWeatherData(data) {
    ...
    const minTempCelsius = (minTempKelvin - 273.15).toFixed(1);
    const maxTempCelsius = (maxTempKelvin - 273.15).toFixed(1);
  
    cityElement.textContent = CITY_NAME;
    tempElement.textContent = `${temperatureCelsius} °C`;
    descElement.textContent = weatherDescription;
    minmaxElement.textContent = `Min: ${minTempCelsius} °C, Max: ${maxTempCelsius} °C`;
  }
   */
